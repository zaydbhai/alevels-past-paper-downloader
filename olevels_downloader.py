#!/usr/bin/env python3
"""
O-Level Past Paper Downloader (v4)
===================================
Fixed subject slugs and year‑link detection for PapaCambridge O‑Level papers.
"""
...
# ------------------------------------------------------------------
# O‑LEVEL SUBJECTS – with CORRECT PapaCambridge slugs
# ------------------------------------------------------------------
SUBJECTS = {
    "4024 - Mathematics (O-Level)":        ("4024", "mathematics-d-calculator-version-4024"),
    "5054 - Physics (O-Level)":            ("5054", "physics-5054"),
    "5070 - Chemistry (O-Level)":          ("5070", "chemistry-5070"),
    "5090 - Biology (O-Level)":            ("5090", "biology-5090"),
    "2281 - Economics (O-Level)":          ("2281", "economics-2281"),
    "7100 - Commerce (O-Level)":           ("7100", "commerce-7100"),
    "7110 - Accounting (O-Level)":         ("7110", "accounting-7110"),
    "7115 - Business Studies (O-Level)":   ("7115", "business-studies-7115"),
    "2210 - Computer Science (O-Level)":   ("2210", "computer-science-2210"),
    "1123 - English Language (O-Level)":   ("1123", "english-language-1123"),
    "2010 - Literature (O-Level)":         ("2010", "literature-in-english-2010"),
    "3247 - Urdu (O-Level)":               ("3247", "urdu-3247"),
    "2059 - Pakistan Studies (O-Level)":   ("2059", "pakistan-studies-2059"),
    "2058 - Islamiyat (O-Level)":          ("2058", "islamiyat-2058"),
}
...
class DownloadWorker(threading.Thread):
    ...
    def run(self):
        ...
        for job_index, (display_name, code, slug, year) in enumerate(self.jobs, start=1):
            if self.stop_event.is_set():
                self.log("[!] Operation cancelled by user.")
                break

            self.log(f"[{job_index}/{len(self.jobs)}] Scanning {display_name} - Year {year}...")

            # CORRECT URL pattern for O‑Level
            subject_url = f"{PAPACAMBRIDGE_BASE}/papers/caie/o-level-{slug}"

            try:
                pdf_entries = self._get_papers_for_year(subject_url, code, year)
            except Exception as exc:
                self.log(f"  ! Error scanning {year}: {exc}")
                continue
            ...
            clean_name = re.sub(r'[\\/*?:"<>|]', "_", display_name).strip()
            subject_folder = f"{code}_{clean_name}"
    ...
    def _get_papers_for_year(self, subject_url, code, year):
        """
        Fetch all PDF links for a specific year.
        Finds year links on the subject page, then scans them for PDFs.
        """
        year_str = str(year)
        short_year = year_str[-2:]

        # ---- Step 1: Get the subject page ----
        try:
            ...
            self.log(f"  ! Failed to fetch subject page: {e}")
            return []

        soup = BeautifulSoup(resp.text, "html.parser")

        # ---- Step 2: Find all links that point to year pages ----
        year_urls = []
        for a in soup.find_all("a", href=True):
            href = a["href"]
            text = a.get_text(strip=True)

            # Look for year in href or link text
            if year_str in href or year_str in text:
                full_url = urljoin(PAPACAMBRIDGE_BASE, href)
                # Skip PDFs and fragment links
                if not full_url.endswith(".pdf") and "#" not in full_url:
                    year_urls.append(full_url)

        # ---- Step 3: If no year links found, try the direct URL ----
        if not year_urls:
            direct_url = subject_url.rstrip('/') + '/' + year_str
            self.log(f"  (No year link found; trying direct URL: {direct_url})")
            year_urls.append(direct_url)

        # ---- Step 4: Scan each year URL for PDFs ----
        found_entries = {}

        for scan_url in year_urls:
            if self.stop_event.is_set():
                break
            ...
            for a in page_soup.find_all("a", href=True):
                href = a["href"]
                # Try to parse from href first, then from link text
                ...
                if clean_name:
                    # Ensure the session part ends with the short year
...
# GUI (unchanged)
# --------------------------------------------------------------------------
class OLevelPaperApp(ctk.CTk):
    def __init__(self):
        super().__init__(className="O'Level Past Paper Downloader")

        self.title("O'Level Past Paper Downloader")
    ...
    def _build_layout(self):
        ...
        ctk.CTkLabel(
            header,
            text="O'Level Past Paper Downloader",
    ...
    def _build_subject_panel(self, parent):
        ...
        for display_name in SUBJECTS:
            self.subject_vars[display_name] = ctk.BooleanVar(value=False)
    ...
    def _refresh_subject_list(self):
        ...
        for display_name, (code, slug) in SUBJECTS.items():
            if query and query not in display_name.lower() and query not in code:
                continue
            ...
            cb = ctk.CTkCheckBox(row, text=display_name, variable=self.subject_vars[display_name])
            cb.pack(side="left", padx=4, pady=6)

    def _set_all_visible(self, value):
        query = self.search_var.get().strip().lower()
        for display_name, (code, slug) in SUBJECTS.items():
            if not query or query in display_name.lower() or query in code:
                self.subject_vars[display_name].set(value)
    ...
    def _build_options_panel(self, parent):
        ...
        default_dir = os.path.join(os.path.expanduser("~"), "OLevelPastPapers")
    ...
    def _start(self):
        ...
        jobs = []
        for display_name, code, slug in subjects:
            for year in years:
                jobs.append((display_name, code, slug, year))
